...
export const getUser: LambdaHandler = async (event, context) => {
  ...
  if (!user) {
    return createResponse(404, {
      message: `No user for id ${event.pathParameters.googleUserId}`
    });
  }
...
export const putUser: LambdaHandler = async (event, context) => {
  ...
  if (
    event.pathParameters.googleUserId !== event.requestContext.authorizer.userId
  ) {
    return createResponse(400, {
      message: "Mismatching user id in path and calling user"
    });
  }

  if (
    user.googleUserId &&
    user.googleUserId !== event.requestContext.authorizer.userId
  ) {
    return createResponse(400, {
      message: "Mismatching user id in body and calling user"
    });
  }
